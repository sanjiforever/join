"""
✅ JOIN REQUEST BOT — python-telegram-bot 21.x
"""
...
async def handle_join_request(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.approve_chat_join_request(chat_id=request.chat.id, user_id=user.id)
        approved_count += 1
        logger.info(f"✅ Tasdiqlandi: {user.full_name} ({user.id})")
        try:
            await ctx.bot.send_message(user.id, "✅ Arizangiz tasdiqlandi!\n\nKanalga muvaffaqiyatli qo'shildingiz. Xush kelibsiz! 🎉")
        except Exception:
            pass
        for aid in ADMIN_IDS:
            try:
                uname = f"@{user.username}" if user.username else "yo'q"
                await ctx.bot.send_message(aid, f"✅ Yangi a'zo!\n👤 {user.full_name}\n🔗 {uname}\n🆔 `{user.id}`\n📊 Jami: {approved_count}", parse_mode="Markdown")
            except Exception:
                pass
